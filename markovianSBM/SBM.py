...
class SBM(RelaxedKmeans, Clustering, Estimation, BaumWelch):
	...
	def __init__(self, n, K, ini_distribution='uniform', framework='iid', X=None, Q=None, P=None, save_B_matrices = False):
		...
		:param ini_distribution: Gives the initial distribution of the chain
		:param framework: Shold be set ot 'iid' if you sample the communities indepently. Otherwise, it should be set to 'markov'
		:param X: Pre-defined adjacency matrix
		:param Q: Connectivity matrix of size $K \times K$
		:param P: Transition matrix of the Markov chain of size $K \times K$
		:param save_B_matrices: It should be set to 'True' if you want to save the output of the relaxed SDP program together with its optimal solution  
		"""
	...
	def initial_distribution(self):
		...
		..note: 
		You can add distribution by yourself in the method
		"""
	...
	def next_state(self, i):
		...
		:param i: Integer representing the community of the current node
		"""
